 ...
 */
public class ChatClient extends java.rmi.server.UnicastRemoteObject // Since we accept remote calls
		...
		RemoteEventListener // So we can receive chat notifications
{
	...
	/**
	 * Helper object (actually, a kind of service) that automates the finding of
	 * interesting Jini services.
	 */
	protected ServiceDiscoveryManager sdm;

	/**
	 * Refers to an object obtained from the service discovery manager to locate a
	 ...
	 */
	protected static final String csi = "dsv.pis.chat.server.ChatServerInterface";
	 ...
	 */
	public ChatClient() throws java.io.IOException, java.lang.ClassNotFoundException, java.rmi.RemoteException {
		...
		chatServiceTemplate = new ServiceTemplate(null, new Class[] { java.lang.Class.forName(csi) }, null);
		...
		sendButton.addActionListener(new ActionListener()
		{
		  public void actionPerformed(ActionEvent e)
		  {
			  if (myServer != null) {
					...
					sendToChat(myName + ": " + textField.getText());
				} else {
...
//		clientPanel.setFont(meiryoFont);
	...
	public JPanel makeButtonPanel() {		
        ...
        //privateMsgButton.addActionListener((ActionListener) this);
	...
	/**
	 * The lookup cache calls this method when a found service has changed its
	 * registration.
	 * 
	 * @param e
	 *            The discovery event.
	...
	/**
	 * The lookup cache calls this method when a found service has removed its
	 * registration.
	 * 
	 * @param e
	 *            The discovery event.
	...
	/**
	 * The ChatServer we are registered with (connected to) calls this method to
	 * notify us of a new chat message.
	 * 
	 * @param rev
	 *            The remote event that is the notification.
	 */
	public void notify(RemoteEvent rev) throws net.jini.core.event.UnknownEventException, java.rmi.RemoteException {
		if (rev instanceof ChatNotification) {
			...
			textArea.append(chat.getText()+"\n");
		}
	}

	/**
	 * This method disconnects the chat client from a chat server. It unregisters
	 * the client and supresses any exceptions generated by the communication.
	 * 
	 * @param server
	 *            The chat server to disconnect from.
	 */
	...
	/**
	 * This method implements the '.connect' user command. If a servername pattern
	 * is supplied, the known chat services are scanned for names in which the
	 * pattern is case-insensitive substring. If no pattern is supplied, the
	 * connection attempt is directed at the first known server (regardless whether
	 * it is answering or not.
	 * 
	 * @param serverNamePattern
	 *            The substring to match against the server name.
	...
	public int connectToChat(String serverNamePattern) {
		...
		if (0 < servers.size()) {
			...
			System.out.println("[No servers matching " + serverNamePattern + " found]");
	...
	/**
	 * This method implements the '.name' user command. It sets the name the user
	 * has choosen for herself on the chat. If the name is null or the empty string,
	 * the &quot;user.name&quot; system property is used as a substitute.
	 *
	 * @param newName
	 *            The user's name.
	 */
	...
	/**
	 * This method implements the send command which is implicit in the command
	 * interpreter (the input line does not start with a period).
	 * 
	 * @param text
	 *            The text to send to the currently connected server.
	 */
	...
	/**
	 * This method implements the '.list' and '.purge' user commands. All known
	 * servers are listed and a call attempt is made with each. Non-reachable
	 * servers are listed and if the purge parameter is true, also removed from the
	 * list of known servers. Note that intermittent network failures (not uncommon
	 * for wireless and mobile users) may cause a service to appear down when it
	 * really is not.
	 *
	 * @param purge
	 *            False to just list the servers, true to remove the service objects
	 *            of services that does not respond.
	 */
	 ...
	 */
	protected String[] cmdHelp = { "Commands (can be abbreviated):",
			...
			"disconnect        Break the connection to the server", "quit              Exit the client",
			"help              This text" };
	 ...
	 * 
	 * @param argv
	 *            Reserved for future used (e.g. '.help purge').
	 */
	...
	/**
	 * Creates a new string which is the concatenation of the elements in a string
	 * array, joined around a given delimiter string.
	 *
	 * @param sa
	 *            The string array to join together.
	 * @param firstIndex
	 *            The index of the first element in sa to consider.
	 * @param delim
	 *            Delimiter string between elements in sa, or null.
	 *
	...
	/**
	 * The user command interpreter. Commands are read from standard input, parsed
	 * and dispatched methods that either alter the client or sends the text to the
	 * ChatServer (when connected).
	 */
	public void readLoop() {
		...
		while (!halted) {
			...
			if (arg.startsWith(".")) {
				...
				String[] argv = (String[]) realWords.toArray(new String[realWords.size()]);
	...
	public static void main(String[] argv)
			throws java.io.IOException, java.lang.ClassNotFoundException, java.rmi.RemoteException {
